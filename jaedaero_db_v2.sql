use jaedaero_db;
-- ============================================================
-- JAEDAERO Database Schema (ERD_v3 기준 + 로컬 DB 변경사항 반영)
-- MySQL 8.0+
-- ============================================================
...
(
    ...
    login_id_encrypted        VARCHAR(1024) NULL COMMENT 'AES 암호화된 기관 로그인 ID',
    login_password_encrypted  VARCHAR(1024) NULL COMMENT 'AES 암호화된 기관 로그인 비밀번호',
    birth_date_encrypted      VARCHAR(1024) NULL COMMENT 'AES 암호화된 생년월일',
...
(
    ...
    last_processed_transaction_id BIGINT NULL COMMENT '거래내역 배치 diff 커서',
...
(
    ...
    input_data_hash CHAR(64) NULL COMMENT 'AI 분석 입력값 SHA-256 해시',
...
(
    ...
    benefit_context ENUM('GENERAL', 'LEAVE') NOT NULL DEFAULT 'GENERAL' COMMENT '혜택 적용 맥락',
...
(
    ...
    CONSTRAINT fk_rebalancing_recommendation_user
        FOREIGN KEY (user_id) REFERENCES users (user_id)
            ON DELETE CASCADE,
    CONSTRAINT chk_rebalancing_recommendation_ratio
        CHECK (recommended_safe_ratio + recommended_balanced_ratio + recommended_aggressive_ratio BETWEEN 99.00 AND 101.00)
) COMMENT='리밸런싱/글라이드패스 추천'
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci;


-- ---------------------------------------------
-- 25. strategy_application : 전략 적용 이력
-- ---------------------------------------------
CREATE TABLE strategy_application
(
    application_id                  BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '전략 적용 ID',
    user_id                         BIGINT    NOT NULL COMMENT '사용자 ID',
    source_type                     ENUM('SIMULATION', 'AI_RECOMMENDATION', 'REBALANCING', 'MANUAL')
...
(
    ...
    title         VARCHAR(255) NOT NULL COMMENT '미션명',
    description   TEXT NULL COMMENT '미션 설명',
    action_type   VARCHAR(50) NOT NULL COMMENT '완료 조건 유형',
    is_repeatable BOOLEAN NOT NULL DEFAULT FALSE COMMENT '반복 미션 여부',
...
-- ---------------------------------------------
-- 28. user_mission_completion : 미션 완료 이력
-- ---------------------------------------------
...
(
    ...
    completed_at   TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '완료 일시',

    CONSTRAINT uq_user_mission_completion UNIQUE (user_id, mission_id, completion_date),
...
(
    ...
    budget_amount BIGINT NULL COMMENT '휴가 예산 설정값',
    created_at    TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성 일시',

    CONSTRAINT fk_leave_mode_user
        FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE,
    CONSTRAINT chk_leave_mode_date CHECK (end_date >= start_date)
) COMMENT='휴가모드 UI 전환 및 휴가 예산 추적'
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci;


-- ---------------------------------------------
-- 30. daily_market_report : 오늘의 시장 리포트
-- ---------------------------------------------
CREATE TABLE daily_market_report
(
    report_id    BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '오늘의 리포트 ID',
    report_date  DATE NOT NULL COMMENT '서비스 기준일',
    content      TEXT NOT NULL COMMENT '시장 경향 리포트',
    valid_from   TIMESTAMP NOT NULL COMMENT '노출 시작 시각',
    valid_until  TIMESTAMP NOT NULL COMMENT '노출 종료 시각',
    created_at   TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성 일시',

    CONSTRAINT uq_daily_market_report_date UNIQUE (report_date),
    CONSTRAINT chk_daily_market_report_period CHECK (valid_until > valid_from)
) COMMENT='오늘의 AI 투자 리포트'
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci;


-- ---------------------------------------------
-- 31. device_token : FCM 디바이스 토큰
-- ---------------------------------------------
...
-- ---------------------------------------------
-- 32. notification_history : 알림 발송 이력
-- ---------------------------------------------
...
(
    ...
    read_at           TIMESTAMP NULL COMMENT '읽음 일시',
...
-- ---------------------------------------------
-- 33. investment_badge : 투자 뱃지
-- ---------------------------------------------
CREATE TABLE investment_badge
(
    badge_id            BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '투자 뱃지 ID',
    user_id             BIGINT        NOT NULL COMMENT '사용자 ID',
    badge_month         DATE NULL COMMENT '월별 수익률 산정 기준 월',
    monthly_return_rate DECIMAL(6, 2) NULL COMMENT '월별 저축+투자 합산 수익률(%)',
    initial_preference  ENUM('SAFE', 'BALANCED', 'AGGRESSIVE') NULL COMMENT '온보딩 투자 성향 시드값',
    badge_tier          ENUM('SAFE', 'BALANCED', 'AGGRESSIVE') NULL COMMENT '뱃지 유형 또는 월간 뱃지 등급',
    badge_grade         ENUM('BRONZE', 'SILVER', 'GOLD', 'PLATINUM', 'DIAMOND') NULL COMMENT '월별 누적 등급',
