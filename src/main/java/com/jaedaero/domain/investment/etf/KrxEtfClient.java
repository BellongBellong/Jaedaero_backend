...
public class KrxEtfClient {
    ...
    public KrxEtfClient(
        ...
        this(URI.create(baseUrl), authKey, HttpClient.newBuilder().connectTimeout(REQUEST_TIMEOUT).build(), new ObjectMapper());
    }
    ...
    public EtfDailyTradingResponse getDailyTrading(String basDd) {
        if (authKey == null || authKey.isBlank()) {
            throw new KrxApiException("KRX API 인증키가 설정되지 않았습니다. application-local.properties에 krx.api.auth-key를 설정하세요.", 503);
        }
        try {
            String requestBody = objectMapper.writeValueAsString(Map.of("basDd", basDd));
            HttpRequest request = HttpRequest.newBuilder(baseUri.resolve(ETF_DAILY_TRADING_PATH))
