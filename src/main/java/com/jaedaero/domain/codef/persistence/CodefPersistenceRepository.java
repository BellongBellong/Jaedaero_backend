...
public class CodefPersistenceRepository {
    ...
    /**
     * The JSP demo has no authentication flow yet. Create its configured local user once so the
     * CODEF connection can still keep the same foreign-key and per-user data model as production.
     */
    public void createDemoUserIfAbsent(long userId) {
        jdbcTemplate.update(
                "INSERT IGNORE INTO users (user_id, social_type, social_id, nickname) VALUES (?, 'DEMO', ?, 'CODEF 데모')",
                userId,
                "codef-demo-" + userId);
    }
    ...
    public Optional<StoredCodefConnection> findConnectionByUserId(long userId) {
        List<StoredCodefConnection> rows = jdbcTemplate.query(
                ...
                (rs, rowNum) -> new StoredCodefConnection(
                        rs.getLong("connection_id"), rs.getLong("user_id"), rs.getString("connected_id_encrypted")),
                userId);
    ...
    public void saveConnection(long userId, String encryptedConnectedId, String connectedIdHash) {
        jdbcTemplate.update(
                ...
                userId, encryptedConnectedId, connectedIdHash);
    }

    public void updateConnectionSyncSuccess(long connectionId) {
        jdbcTemplate.update(
                "UPDATE codef_connection SET last_sync_at = CURRENT_TIMESTAMP, last_sync_error_message = NULL "
                        + "WHERE connection_id = ?",
                connectionId);
    }
    ...
    public List<StoredConnectedAccount> findAccountsByUserId(long userId) {
        return jdbcTemplate.query(accountSelect() + " WHERE cc.user_id = ? AND ca.status = 'ACTIVE' ORDER BY ca.account_id",
                this::mapAccount, userId);
    }

    public List<StoredConnectedAccount> findAccountsByUserIdAndInstitution(long userId, String institutionCode) {
        return jdbcTemplate.query(
                accountSelect() + " WHERE cc.user_id = ? AND ca.institution_code = ? "
    ...
    public Optional<StoredConnectedAccount> findAccountByIdAndUserId(long accountId, long userId) {
        List<StoredConnectedAccount> rows = jdbcTemplate.query(
                accountSelect() + " WHERE ca.account_id = ? AND cc.user_id = ? AND ca.status = 'ACTIVE'",
                this::mapAccount,
                accountId,
                userId);
        return rows.stream().findFirst();
    ...
    public boolean isTransactionPeriodCovered(
        ...
        Integer count = jdbcTemplate.queryForObject(
                ...
                Integer.class,
                accountId, inquiryType, toSqlDate(startDate), toSqlDate(endDate));
        return count != null && count > 0;
    }
    ...
    public void recordTransactionSyncPeriod(
            long accountId, String inquiryType, LocalDate startDate, LocalDate endDate) {
        jdbcTemplate.update(
                "INSERT INTO account_transaction_sync (account_id, inquiry_type, requested_start_date, requested_end_date) "
    ...
    public void insertTransactionIfAbsent(
        ...
        jdbcTemplate.update(
                "INSERT IGNORE INTO transaction_history (account_id, transaction_datetime, amount, balance_after, "
                        + "transaction_type, transaction_description, external_transaction_key) VALUES (?, ?, ?, ?, ?, ?, ?)",
                accountId,
                transactionAt,
                amount,
                balanceAfter,
    ...
    public void upsertSoldierSaving(
        ...
        jdbcTemplate.update(
                ...
                userId, accountId, bankName, monthlyAmount, interestRate, toSqlDate(startDate), toSqlDate(endDate));
    }

    public List<StoredTransaction> findTransactions(long accountId, LocalDate startDate, LocalDate endDate) {
        return jdbcTemplate.query(
                ...
                (rs, rowNum) -> new StoredTransaction(
                        rs.getLong("transaction_id"), rs.getTimestamp("transaction_datetime").toLocalDateTime(),
                        rs.getLong("amount"), rs.getObject("balance_after", Long.class),
                        rs.getString("transaction_type"), rs.getString("category"),
    ...
    private StoredConnectedAccount mapAccount(java.sql.ResultSet rs, int rowNum) throws java.sql.SQLException {
