...
public class CodefTransactionSyncService {
    ...
    public CodefTransactionSyncService(CodefApiClient codefApiClient, CodefPersistenceRepository repository,
    ...
    public int sync(long userId, long accountId, String startDate, String endDate) {
        ...
        for (JsonNode row : rows) {
            ...
            String legacyKeyDescription = firstText(
                    row, "resAccountDesc4", "resAccountDesc3", "resAccountDesc2", "tranDesc");
            String key = hasher.hash(accountNumber + "|" + date + "|" + time + "|" + income + "|" + outcome
                    + "|" + row.path("resAfterTranBalance").asText() + "|" + legacyKeyDescription);
            repository.insertTransactionIfAbsent(accountId, transactionAt, amount,
                    nullableNumber(row.path("resAfterTranBalance")), type, description, key);
            count++;
        }
        return count;
    }
