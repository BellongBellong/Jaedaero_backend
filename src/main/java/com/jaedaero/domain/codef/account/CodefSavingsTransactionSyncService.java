...
public class CodefSavingsTransactionSyncService {
  ...
  public int sync(long userId, long accountId, String startDate, String endDate) {
    ...
    JsonNode data =
        codefApiClient.postProduct(INSTALLMENT_SAVINGS_TRANSACTION_LIST_PATH, body).path("data");
  ...
  private void syncSoldierSavingIfMatched(
      long userId, StoredConnectedAccount account, JsonNode data) {
  ...
  private String description(JsonNode row) {
    String transactionType =
        firstText(row, "resAccountDesc3", "resAccountDesc2", "resAccountDesc1");
