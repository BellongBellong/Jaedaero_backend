...
public class CodefConnectionService {
  ...
  public CodefConnectionResponse connect(long userId, CodefBankConnectionCreateRequest request) {
        ...
        .map(
            connection -> {
              repository.saveInstitutionConnection(
                  ...
                  cipher.encrypt(request.getLoginId()),
    ...
    StoredCodefConnection existingConnection =
        repository.findConnectionByUserId(userId).orElse(null);
    CodefAccountCreateResponse codefResponse =
        existingConnection == null
            ? accountClient.createAccount(request.toCodefRequest())
            : accountClient.addAccount(
                cipher.decrypt(existingConnection.connectedIdEncrypted()),
                request.toCodefRequest());
    ...
    return new CodefConnectionResponse(
        ...
        organizationCode,
        count,
        codefResponse.getSuccessList(),
        codefResponse.getErrorList());
  }

  @Transactional
  public int syncRegisteredInstitution(
      long userId, String organizationCode, String businessTypeCode) {
    CodefBusinessType businessType = CodefBusinessType.fromCode(businessTypeCode);
    validateInstitution(organizationCode, businessType);
    repository
        .findInstitutionConnection(userId, organizationCode, businessType.getCode())
        .filter(connection -> "ACTIVE".equals(connection.status()))
