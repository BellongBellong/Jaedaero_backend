...
public class AiAnalysisServiceImpl implements AiAnalysisService {
  ...
  private AiRecommendedScenarioVo recommendation(long userId, AiAnalysisInput base, SimulationVo simulation, LocalDate today) {
    ...
    long requestedSaving =
        simulation == null ? extra : simulation.getMonthlySavingAmount() + extra;
    long saving =
        Math.min(
            Math.max(0L, requestedSaving),
            Math.min(
                SimulationAllocationPolicy.MAX_MONTHLY_SAVING_AMOUNT,
                referenceMonthlyIncome));
    long requestedSpending =
        simulation == null
            ? base.monthlySpendingLimit()
            : Math.max(
                0,
                simulation.getMonthlySpendingAmount()
                    - Math.min(30_000, simulation.getMonthlySpendingAmount() / 10));
    long spending =
        Math.min(
            Math.max(0L, requestedSpending),
            Math.max(0L, referenceMonthlyIncome - saving));
    long requestedInvestment =
        simulation == null
            ? amountFromRate(referenceMonthlyIncome, DEFAULT_INVESTMENT_RATE)
            : simulation.getMonthlyInvestmentAmount();
    long monthlyInvestmentAmount =
        Math.min(
            Math.max(0L, requestedInvestment),
            Math.max(0L, referenceMonthlyIncome - saving - spending));
    ...
    return AiRecommendedScenarioVo.builder().userId(userId).monthlySavingAmount(saving).monthlySpendingAmount(spending)
        .monthlyInvestmentAmount(monthlyInvestmentAmount).expectedReturnRate(expectedReturn).expectedAsset(calculated.expectedAsset())
  ...
  private AiAnalysisResult result(AiAnalysisInput base, SimulationVo simulation, AiRecommendedScenarioVo recommended) {
    ...
    return AiAnalysisResult.builder().financialDischargeDate(financial).deltaDaysVsActual(days(base.actualDischargeDate(), financial))
        .achievementRate(rate(expected, targetAmount)).comment(comment(base, expected, scenario, targetAmount))
  ...
  private String reason(AiAnalysisInput base, long saving, long spending, long targetAmount) {
    long shortfall = Math.max(0, targetAmount - base.expectedAsset());
    return shortfall == 0
        ? "현재 목표 달성 페이스를 유지할 수 있도록 소비 한도를 관리하는 추천입니다."
        : "목표까지 "
            + money(shortfall)
            + "원이 부족해 장병내일준비적금 월 납입 "
            + money(saving)
            + "원, 월 소비 "
            + money(spending)
            + "원 이하를 제안합니다.";
  ...
  private String prompt(AiAnalysisInput base, SimulationVo simulation, AiAnalysisResult result, AiRecommendedScenarioVo recommended) {
    ...
    return "[분석 기준] " + (simulation == null ? "평소 자산 흐름" : "What-if 시뮬레이션 비교") + "\n"
        ...
        + "추천 장병내일준비적금 월 납입액: " + money(recommended.getMonthlySavingAmount()) + "원\n"
  ...
  private long amountFromRate(long amount, BigDecimal rate) {
