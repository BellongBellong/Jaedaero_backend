...
public class EtfMarketOverviewService {
    ...
    public EtfMarketOverviewResponse getOverview(LocalDate requestedAsOfDate) {
        ...
        Map<String, EtfDailyTradingInfo> sixMonths = byCode(findEarliestDailyAtOrAfter(asOfDate.minusMonths(6)));
        Map<String, EtfDailyTradingInfo> yearToDate = byCode(findEarliestDailyAtOrAfter(asOfDate.withDayOfYear(1)));
        Map<String, EtfDailyTradingInfo> oneYear = byCode(findEarliestDailyAtOrAfter(asOfDate.minusYears(1)));
        Map<String, EtfDailyTradingInfo> twoYears = byCode(findEarliestDailyAtOrAfter(asOfDate.minusYears(2)));

        List<EtfMarketOverviewItem> items = asOfItems.stream()
                .map(item -> new EtfMarketOverviewItem(item, List.of(
                        calculate("SIX_MONTHS", asOfDate.minusMonths(6), item, sixMonths.get(item.isuCd())),
                        calculate("YEAR_TO_DATE", asOfDate.withDayOfYear(1), item, yearToDate.get(item.isuCd())),
                        calculate("ONE_YEAR", asOfDate.minusYears(1), item, oneYear.get(item.isuCd())),
                        calculate("TWO_YEARS", asOfDate.minusYears(2), item, twoYears.get(item.isuCd())))))
                .toList();
        return new EtfMarketOverviewResponse(format(requestedAsOfDate), format(asOfDate), items);
    }

    private EtfReturnPeriod calculate(
            String period, LocalDate requestedBaseDate, EtfDailyTradingInfo current, EtfDailyTradingInfo base) {
        if (base == null) {
        ...
        try {
            ...
            BigDecimal returnRate = currentPrice.subtract(basePrice).multiply(BigDecimal.valueOf(100))
                    .divide(basePrice, 2, RoundingMode.HALF_UP);
            return new EtfReturnPeriod(period, format(requestedBaseDate), base.basDd(), base.tddClsprc(), returnRate, true);
