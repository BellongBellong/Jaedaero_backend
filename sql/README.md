...
`jaedaero_db_v1.sql`은 V3 계산 구조와 v2 스키마 보완을 합친 최신 기준 파일입니다. 기존 테이블을 모두 삭제하므로 신규 DB 또는 전체 초기화가 확정된 환경에서만 실행합니다.

```bash
mysql -u <DB_USER> -p <DB_NAME> < jaedaero_db_v1.sql
```

v1에는 V3 계산 구조와 2026-08-13까지의 스키마 마이그레이션 결과가 반영되어 있습니다. v1을 실행한 신규 DB에는 이미 반영된 스키마용 `sql/migration` 파일을 다시 실행하지 않습니다.
...
운영 중인 DB에는 `jaedaero_db_v1.sql`을 실행하지 않습니다. 먼저 백업한 뒤 아직 적용하지 않은 `sql/migration` 파일만 날짜순으로 실행합니다.

- `ALTER TABLE` 기반 파일은 대부분 1회 실행 전용입니다.
- `20260811_backfill_user_badges.sql`은 스키마 변경이 아닌 기존 데이터 보정용입니다.
- `20260813_add_mission_code.sql`과 `20260813_expand_leave_mode_event.sql`까지 v1 기준 스키마에 반영되어 있습니다.
...
| 구분 | 역할 | 재실행 |
| --- | --- | --- |
| `jaedaero_db_v1.sql` | 신규 DB 기준 스키마 | 전체 초기화할 때만 |
...
- `./gradlew dbMigrate -PconfirmLocalDb=<DB명>` — 확인한 로컬 DB의 기존 데이터를 삭제하고 v1 스키마만 재생성
- `./gradlew dbSeed` — 운영 기준 데이터만 적재
- `./gradlew dbSeedMock -PconfirmLocalDb=<DB명>` — 운영 기준 데이터 적용 후 개발·QA 목데이터 적재
- `./gradlew dbReset -PconfirmLocalDb=<DB명>` — v1 스키마 재생성 후 운영 기준 데이터 적재
- `./gradlew dbResetWithMock -PconfirmLocalDb=<DB명>` — v1 스키마와 운영 기준 데이터, 목데이터까지 모두 적재
